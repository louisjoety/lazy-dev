...
def id_to_output(ids: list, user_prompt):
    ...
    def ai_code_picker(source_code, user_prompt):
        ...
        files = [
            ...
            client.files.upload(file="jsontester.txt"),
        ]
        model = "gemini-2.0-flash"
        contents = [
            types.Content(
                ...
                parts=[
                    ...
                    types.Part.from_text(text="""Show me where I used csv library"""),
                ],
            ),
            types.Content(
                role="model",
                parts=[
                    types.Part.from_text(text="""readwise.py:
    ...
    RESPONSE FORMAT: filename: relevant code snippets. Ensure there is only one \"filename:\" for the snippets from the same file
    ...
    for id in ids:
        source_code = supabase_table[id]["source_code"]  #TODO: Change this to an actual supabase query

        # Appending the relevant code to snippets with project metadata
        snippets = snippets + ai_code_picker(source_code, user_prompt)
